...
function delete_snippet(snippet) {
    if(confirm('Are you sure you want to delete this snippet?')) {
        $.post('remove.php?file='+encodeURI(snippet),function(output) {
...
function execute(form) {
    ...
    $.post('run.php',form.serialize(),function(output) {
        $('#livephp_output').html(output);
    });
}

function load(snippet, callback) {
    if(!snippet)
        return false;
    
    $('#livephp_output').html('Loading ...');
    $.get('load.php?file='+encodeURI(snippet),function(code) {
...
function get_url(noselect) {
    ...
    var key=window.btoa(unescape(encodeURIComponent($('#livephp_snippet_name').val()))),
        url=window.location.href.replace(/\/live-php\/.*$/,'')+'load/'+key;
...
function save() {
    ...
    $.post('save.php',$('form').serialize(),function(output) {
        $('#livephp_output').html(output);
        load_all();
    });
}
